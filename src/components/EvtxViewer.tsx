...
export function EvtxViewer({
	...
	const records: ParsedEventRecord[] = isParsedMode
		...
		: parserRecords
	const recordCount = isParsedMode
		? parsedResult.records.length
		: parserRecordCount
	...
	const result: EvtxParseResult | null = isParsedMode
		...
		: state.status === 'done'
			? state.result
			: null
	const effectiveFileName = isParsedMode
		? propFileName || 'unknown'
		: state.status === 'done'
			? state.fileName
			: ''
	const effectiveFileSize = isParsedMode
		? propFileSize || 0
		: state.status === 'done'
			? state.fileSize
			: 0
	const effectiveParseTime = isParsedMode
		? propParseTime || 0
		: state.status === 'done'
			? state.parseTime
			: 0
	...
	const statusMessage =
		state.status === 'reading'
			? 'Reading file...'
			: state.status === 'parsing'
				? `Parsing... ${state.recordCount.toLocaleString()} records`
				: state.status === 'done'
					? `Parsed ${state.result.totalRecords.toLocaleString()} event records`
					: state.status === 'error'
						? state.error
						: ''
	...
	const handleClick = useCallback(() => {
		...
		a.download = `${effectiveFileName}-rawdump.xml`
		a.click()
		URL.revokeObjectURL(a.href)
	}, [buildXml, effectiveFileName])
				...
				<>
					...
					{isDone && result && (
						<Group gap='sm' style={{width: '100%'}}>
							...
							<Button
							onClick={() => clipboard.copy(buildXml())}
							size='sm'
							variant='default'
						>
